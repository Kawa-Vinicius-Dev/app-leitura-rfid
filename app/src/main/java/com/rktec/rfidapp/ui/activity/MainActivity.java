...
import com.rktec.rfidapp.importacao.ImportadorPlanilha;
import com.rktec.rfidapp.importacao.ImportadorSetor;
import com.rktec.rfidapp.model.ItemPlanilha;
import com.rktec.rfidapp.util.MapeadorSetor;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String nome = getSharedPreferences("prefs", MODE_PRIVATE).getString("usuario_nome", null);
        if (nome == null) {
        ...
        if ("CEO".equalsIgnoreCase(permissao)) {
            ...
            btnGerenciarUsuarios.setVisibility(View.VISIBLE);
            btnGerenciarUsuarios.setOnClickListener(v -> startActivity(new Intent(MainActivity.this, GerenciarUsuariosActivity.class)));
        } else {
        ...
        // ===================== IMPORTAR PLANILHA =====================
        importarPlanilhaLauncher = registerForActivityResult(new ActivityResultContracts.GetContent(), uri -> {
            if (uri == null) return;
            ...
            new Thread(() -> {
                ...
                if (importada != null && !importada.isEmpty() && listaSetores != null && !listaSetores.isEmpty()) {
                    ...
                    android.util.Log.d("MainActivity", "Mapeamento após importar planilha levou " + (fimMap - inicioMap) + " ms");
                }
                ...
                runOnUiThread(() -> {
                    listaPlanilha = (resultado != null) ? resultado : new ArrayList<>();
                    ...
                    imgIconPlanilha.setColorFilter(ContextCompat.getColor(MainActivity.this, R.color.success_green));

                    btnLimparPlanilha.setVisibility(View.VISIBLE);

                    Toast.makeText(MainActivity.this, "Importados " + listaPlanilha.size() + " itens! (total " + duracaoTotal + " ms)", Toast.LENGTH_SHORT).show();
        ...
        // ===================== IMPORTAR SETORES =====================
        importarSetorLauncher = registerForActivityResult(new ActivityResultContracts.GetContent(), uri -> {
            if (uri == null) return;
            ...
            new Thread(() -> {
                ...
                if (importados != null && !importados.isEmpty() && listaPlanilha != null && !listaPlanilha.isEmpty()) {
                    ...
                    long fimMap = System.currentTimeMillis();
                    android.util.Log.d("MainActivity", "Mapeamento após importar setores levou " + (fimMap - inicioMap) + " ms");
                }
                ...
                runOnUiThread(() -> {
                    listaSetores = (resultado != null) ? resultado : new ArrayList<>();
                    ...
                    imgIconSetor.setColorFilter(ContextCompat.getColor(MainActivity.this, R.color.success_green));

                    btnLimparSetor.setVisibility(View.VISIBLE);

                    Toast.makeText(MainActivity.this, "Importados " + listaSetores.size() + " setores! (total " + duracaoTotal + " ms)", Toast.LENGTH_SHORT).show();
    ...
    private void mostrarDialogoDeLimpeza(String tipoArquivo) {
        ...
        String mensagem = limparPlanilha ? "Tem certeza que deseja descartar a planilha importada? Você precisará selecionar o arquivo novamente." : "Tem certeza que deseja descartar o arquivo de setores? Você precisará selecionar o arquivo novamente.";

        tvTitle.setText(titulo);
        tvMsg.setText(mensagem);

        AlertDialog dialog = new AlertDialog.Builder(this, R.style.AppDialogTheme).setView(dialogView).setCancelable(true).create();
    ...
    private void resetarEstadoBotaoPlanilha() {
        ...
        imgIconPlanilha.setColorFilter(ContextCompat.getColor(this, R.color.novo_atacarejo_blue));
    ...
    private void resetarEstadoBotaoSetor() {
        ...
        imgIconSetor.setColorFilter(ContextCompat.getColor(this, R.color.novo_atacarejo_blue));

        btnLimparSetor.setVisibility(View.GONE);
        Toast.makeText(this, "Setores descartados.", Toast.LENGTH_SHORT).show();
    }

    public void onImportarPlanilha(View v) {
        importarPlanilhaLauncher.launch("text/*");
    }

    public void onImportarSetor(View v) {
        importarSetorLauncher.launch("text/*");
    }

    public void onEscolherLoja(View v) {
        if (listaPlanilha == null || listaPlanilha.isEmpty() || listaSetores == null || listaSetores.isEmpty()) {
    ...
    public void onLogout(View v) {
        ...
        AlertDialog dialog = new AlertDialog.Builder(this, R.style.AppDialogTheme).setView(dialogView).setCancelable(false).create();
